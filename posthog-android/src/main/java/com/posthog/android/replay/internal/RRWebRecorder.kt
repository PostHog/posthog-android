...
internal class RRWebRecorder : PostHogRecorder {
    companion object {
    ...
    val recording = mutableListOf<RREvent>()
    private var currentFrame = RREvent(0, 0)
    private var currentFrameCommands = mutableListOf<Map<String, Any>>()
    ...
    override fun beginFrame(timestampMs: Long, width: Int, height: Int) {
        if (recording.isEmpty()) {
            ...
            recording.add(RREvent(timestampMs, TYPE_DOMCONTENTLOADEDEVENT))

            // LoadEvent
            recording.add(RREvent(timestampMs, TYPE_LOADEVENT))
        ...
        currentFrameCommands = mutableListOf()
    ...
    private fun setupPaint(paint: Paint) {
        ...
        colorFilter?.let {
            if (it is PorterDuffColorFilter) {
                ViewHelper.decodePorterDuffcolorFilter(it)?.let { (color, _) ->
                    currentFrameCommands.add(
                        mapOf(
                            "property" to "fillStyle",
                            "args" to listOf(color),
