 ...
 * Ported from posthog-python/posthog/feature_flags.py
 */
internal class FlagEvaluator(
    ...
    companion object {
        ...
        private fun casefold(input: String): String {
            return REGEX_COMBINING_MARKS.replace(input.toString().uppercase().lowercase(), "")
        }
    }
    ...
    fun matchProperty(
        ...
        return when (operator) {
            ...
            "icontains" -> stringContains(
                overrideValue.toString(),
                value.toString(),
                ignoreCase = true
            )

            "not_icontains" -> !stringContains(
                overrideValue.toString(),
                value.toString(),
                ignoreCase = true
            )
     ...
     * Supports various formats mirroring Python's dateutil.parser.parse():
     * - ISO 8601: "2024-01-01T00:00:00Z"
     * - ISO date only: "2022-05-01"
     * - Datetime with space + timezone: "2022-04-05 12:34:12 +01:00"
     * - Datetime with timezone (no space): "2022-04-05 12:34:12+01:00"
     * - Datetime without timezone: "2022-05-01 00:00:00"
     */
    ...
    fun matchCohort(
        ...
        val cohortId = property.value?.toString()
        ...
        val propertyGroup = cohortProperties[cohortId]
